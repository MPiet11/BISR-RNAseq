...
sed -i 's/Partial Novel Splicing Junctions:/OneSample\tPartial_Novel_Splicing_Junctions\t/' temp_junc
sed -i 's/Novel Splicing Junctions:/OneSample\tNovel_Splicing_Junctions\t/' temp_junc
sed -i 's/Known Splicing Junctions:/OneSample\tKnown_Splicing_Junctions\t/' temp_junc
sed -i 's/Total splicing  Junctions:/OneSample\tTotal_Splicing_Junctions\t/' temp_junc
#sed -i 's/\.e.......\t/\t/' temp_junc
awk   -v OFS='\t' '{print $1,$2,$3}' temp_junc  > rseqc_junctions.txt

grep ribo rnaseq_* > temp_ribo
sed -i 's/ (/\t/' temp_ribo
sed -i 's/):/\t/' temp_ribo
sed -i 's/ /_/g' temp_ribo
awk  -v OFS='\t' '{print "OneSample",$2,$3}' temp_ribo > rseqc_ribo.txt
...
awk  -v OFS='\t'  '{print $1,$2,$11}' hisat2_metrics.txt > rseqc_hisat2_overall_alignment.txt
awk -v OFS='\t' '{print $1,$4*2+$6*2+$9,$5*2+$10,$8}' hisat2_metrics.txt > rseqc_hisat2_uniq_multi_unmap.txt
 sed -i 's/0\t0\tAligned_0_time/UniquelyMapped\tMultiMapped\tUnMapped/'  rseqc_hisat2_uniq_multi_unmap.txt
...
mkdir rdata
Rscript /fs/project/PCON0005/BISR/workflows/RNAseq_pipeline_current/read_data_for_rshiny.R
